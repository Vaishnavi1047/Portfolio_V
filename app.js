const boxContainer = document.querySelector(".activity-boxes");
const greetingTextContainer = document.querySelector(".greeting-text-container");
const authorName = document.querySelector(".name");
const jobTitleSections = document.querySelectorAll(".job-title-section");
const jobTitles = document.querySelectorAll(".job-title");
const projectsHeading = document.querySelector(".projects-heading");
const laptopFrame = document.querySelectorAll(".laptop-frame");
...
for (let i = 0; i < 365; i++) {
    ...
    el.classList = list.includes(i) ? "activity-box active" : "activity-box";
    boxContainer.appendChild(el);
}

window.addEventListener("scroll", () => {
    let offsetY = window.scrollY;
    greetingTextContainer.style.transform = `translateX(${offsetY * 0.5}px)`;
    authorName.style.transform = `translateX(${offsetY * 0.2}px)`;
    jobTitleSections[0].style.backgroundPositionY = `${offsetY * 0.5}px`;
    jobTitleSections[1].style.backgroundPositionY = `${-offsetY * 0.5}px`;
    ...
    laptopFrame[0].style.transform = `translateX(calc(500vh - ${offsetY}px))`;
    laptopFrame[1].style.transform = `translateX(calc(600vh - ${offsetY}px))`;
    //laptopFrame[2].style.transform = `translateX(calc(750vh - ${offsetY}px))`;
});
